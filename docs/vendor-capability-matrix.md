...
| Runtime / custom variables (per-call context) | `VERIFIED` | `[openapi]` | `custom_data` (key→string) on single & bulk calls; agent exposes `custom_variables`/`required_variables`. **Supports the stage-driven "reuse agent + inject context" model — no per-candidate agents.** **Templating convention (F-009):** generated agent scripts use **single-brace** placeholders (`{candidate_name}`, `{persona_name}`, `{company}`, `{job_role}`), matching the account's existing agents — not `{{ }}`. The platform injects these via `custom_data` at call time; keep `app/integrations/hunar/agent_spec.py` in sync with this convention. |
