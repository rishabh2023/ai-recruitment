...
| F-007 | Settings — org config (provider API keys, default provider, live-calling, team + invites) | Done | Standard | Phase 6 | `apps/api/app/modules/organizations/{models,settings_service,invites}.py` (migrations `c2d3e4f5a6b7`, `d3e4f5a6b7c8`), `apps/api/app/api/routers/{settings,auth}.py`, `apps/web/app/{settings,accept-invite}/` | `org_settings`+`user_invites` tables; admin-only settings + team invites (one-time accept link → set password → join); write-only provider keys; live-calling toggle; org-first key resolution. **94 API/Python tests pass**; web typechecks; browser-verified (invite → accept → joined) |
| F-008 | MCP server — conversational access (Claude Code / ChatGPT) | Done | Standard | Phase 6 | `apps/mcp/` (`server.py`, `requirements.txt`, `README.md`), root `.mcp.json` | 29 MCP tools wrapping the backend (jobs, workflow, candidates, sourcing, outreach, settings) incl. macros `source_and_outreach` (Flow B) and `import_and_interview` (Flow A); stdio + streamable-http; cookie auth w/ auto-relogin. Verified live through MCP: real PDL search→add, macros, JD-extract, timeline; HTTP transport boots at `/mcp` |
