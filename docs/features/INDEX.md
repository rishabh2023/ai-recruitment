...
| F-006 | Sourcing — People Search & Outreach (search → add → enrich → outreach) | Done (search+add+enrich+outreach); real Apollo enrichment = Phase 4b | Standard | Phase 5 | `apps/api/app/modules/sourcing/`, `apps/api/app/api/routers/sourcing.py`, `apps/api/app/integrations/people_search/{sample,apollo,base}.py`, `apps/web/app/sourcing/`, `apps/web/app/job-candidates/[id]/` | JD-derived query → provider search (Apollo w/ offline `sample` fallback, flagged) → add (SOURCED, deduped) → enrich contact (sample/degraded, OUTREACH_PENDING) → outreach call (CONTACTED). **85 API/Python tests pass**; web builds+typechecks; browser-verified end to end |
