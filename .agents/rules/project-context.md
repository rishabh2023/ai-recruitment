# Antigravity — Project Context Rule

You are working in the **AI Recruitment Workflow** repository. The Git repo is the durable
source of truth, not this conversation. Shared rules live in the documents below — this rule
points you to them and does not duplicate them.
...
- Before ending an incomplete session or switching agents, write a handoff in
  `docs/work/active-feature.md` so another agent can continue safely.
- Report verification evidence and remaining limitations before claiming completion.

## Shared workflows

Step-by-step procedures live in `.agents/workflows/`: `implement-feature.md` and
`review-feature.md`.
