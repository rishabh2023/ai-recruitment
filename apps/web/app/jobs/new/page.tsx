    ...
    <main className="container job-create">
      <div className="job-create-header">
        <div>
          ...
          <p className="eyebrow">New hiring workflow</p>
          <h1>Create a job</h1>
          <p className="muted">Set the role, review the AI draft, then approve a workflow your team can run with.</p>
        </div>
        <ol className="wizard-progress" aria-label="Job creation progress">
          ...
          <li className={workflow?.approved ? "complete" : version?.confirmed ? "current" : ""} aria-current={currentStep === 3 ? "step" : undefined}>3 · Workflow</li>
      ...
      <div className="job-create-layout">
        <aside className="create-context">
          ...
          <p>{!job ? "A clear title anchors the job description, workflow, and candidate journey." : !version?.confirmed ? "Paste a description or upload a PDF. You will review AI extraction before it is used." : !workflow?.approved ? "Your team can customize every stage before a recruiter approves it." : "The workflow is approved. Activate the role when you are ready to accept candidates."}</p>
          <div className="create-context-steps"><span className={job ? "done" : "active"}>01 <b>Role</b></span><span className={version?.confirmed ? "done" : job ? "active" : ""}>02 <b>Job details</b></span><span className={workflow?.approved ? "done" : version?.confirmed ? "active" : ""}>03 <b>Workflow</b></span><span className={job?.status === "active" ? "done" : workflow?.approved ? "active" : ""}>04 <b>Activate</b></span></div>
        </aside>
      ...
      {job && (
        <section className={`wizard-step ${version?.confirmed ? "done" : ""}`}>
          ...
          <div className="step-heading"><div><p className="eyebrow">Role context</p><h2>Job description</h2><p>Use text or a PDF. You will confirm the extracted details before we draft a workflow.</p></div>{version?.confirmed && <span className="step-status">Confirmed</span>}</div>
          {!version ? (
      ...
      {version?.confirmed && (
        <section className={`wizard-step workflow-step ${workflow?.approved ? "done" : ""}`}>
          ...
          <div className="step-heading"><div><p className="eyebrow">Recruiter review</p><h2>Hiring workflow</h2><p>Customize stages and success criteria before this workflow can be used.</p></div>{workflow?.approved && <span className="step-status">Approved</span>}</div>
          {!workflow ? (
            ...
            >{busy ? "Drafting…" : "Draft workflow"}</button></div>
          ) : (
            <>
              {!workflow.approved ? (
                <>
                  ...
                  <div className="workflow-approval">
                    <div><strong>Ready to make this workflow live?</strong><p>After approval, stage changes require a new workflow version.</p></div>
                    <button disabled={busy} onClick={() => run(async () => setWorkflow(await api.approveWorkflow(job!.id, workflow.id)))}>
                      {busy ? "Approving…" : "Approve workflow"}
                    </button>
                  </div>
                </>
              ) : (
      ...
      {workflow?.approved && (
        <section className={`wizard-step ${job?.status === "active" ? "done" : ""}`}>
          ...
          <div className="step-heading"><div><p className="eyebrow">Launch</p><h2>Activate job</h2><p>Make this approved workflow available to your recruiting team.</p></div>{job?.status === "active" && <span className="step-status">Active</span>}</div>
          {job?.status !== "active" ? (
            ...
            <div className="step-card completed-card"><div><strong>Job is active</strong><p>Your team can now add candidates and begin the hiring workflow.</p></div><Link className="btn" href={`/jobs/${job.id}/candidates`}>Add candidates</Link></div>
          )}
        </section>
      )}
        </div>
